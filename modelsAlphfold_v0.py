...
def train_model(model, train_loader, val_loader=None, epochs=50, lr=0.01, modelname=""):
    ...
    # Save the weights
    torch.save(model.state_dict(), "results/" + modelname + "_protein_model_Alphafold.pth")
    print("Model saved!")
#3. Prediction Script
...
df_pos = pd.read_csv('Data/Alphafold/positiveFeatures.csv')
###Remove the data with NA
#df_pos = df_pos[~np.isnan(np.sum(df_pos,axis=1))]
print(df_pos.shape)
df_neg = pd.read_csv('Data/Alphafold/negativeFeatures.csv')
###Remove the data with NA
#df_neg = df_neg[~np.isnan(np.sum(df_neg,axis=1))]
print( df_neg.shape)
...
X_backup = X
check = np.sum(X_backup, axis=1)
X = X_backup[~np.isnan(check),]
y = y[~np.isnan(check),]
##Let us try 10 times
resultStr = ""
for seedrandom in range(2):
    ...
    y_score = predict(model, X1_test, X2_test, "results/seed-" + str(seedrandom) + "_protein_model_Alphafold.pth")
...
print(resultStr)
with open('results/result_Alphafold.txt', 'w') as f:
    f.write(resultStr)
print("Hello World")